...
CheckJDK()
{
  ...
  # We want to igonre 'SetJavaHome @JAVA_HOME@', but give errors for any other unknown Java Home
  ...
  then
    ...
    then
      ...
      echo "The JDK was specified by a SetJavaHome directive in a .conf file or by a --setjavahome option."
    else
      if [ "X$APP_JAVA_HOME" != "X" ]
      then
        if [ "$hasMinimumMajorJavaVersion" = "1" ]
...
AppendVMSpecificOptions()
{
  ...
  if [ ! "${APP_JAVA_MAJOR_VERSION}" -lt "9" ]
  then
    EchoIfVerbose "Adding VM9 and higher options"
    ...
    then
      EchoIfVerbose "Adding VM9 and higher Linux options"
    ...
    then
      EchoIfVerbose "Adding VM9 and higher MAC options"
    ...
    then
      EchoIfVerbose "Adding VM9 and higher Solaris options"
  ...
  then
    EchoIfVerbose "Adding VM9 and 10 options"
  ...
  then
    EchoIfVerbose "Adding VM11 and higher options"
    for (( i = 0; i < ${#APP_JAVA11_OR_HIGHER_LIBS[@]}; i++ )); do
      AddJavaLibFile "${APP_JAVA11_OR_HIGHER_LIBS[$i]}"
    done
    if [ "darwin" = "$machineUname" ]
    then
      EchoIfVerbose "Adding VM11 and higher MAC options"
    ...
    if [ "linux" = "$machineUname" ]
    then
      EchoIfVerbose "Adding VM11 and higher Linux options"
  ...
  then
    EchoIfVerbose "Adding VM9 thru 12 options"
...
{
  EchoIfVerbose "Adding VM options specified on the commandline with -J"
      ...
      do
        case $1 in
          ...
          -J*)
            ...
            EchoIfVerbose "* Added Commandline VM Option ${NEWARG}"
          ;;
          -debug)
            SetDebug
          ;;
